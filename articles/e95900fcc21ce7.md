...
Changes to Outputs:
  - vpc_id = "vpc-0c1962cc15d60eef0" -> null
```
リソースの削除、および再作成されることがわかります。

### movedを使う

今までは、リソースを変更することなくリファクタリングするには、 `terraform state mv` を
使うか、`tfstate` を手動で変更するしかなかったのですが、 `1.1` 以降のバージョンでは、
**次の様に構造の変更を宣言的に記述することが出来ます。**
...
```
この様に変更が発生しないことが分かります。

さらに `moved` ブロックは `module` の命名の変更にも対応しています。 
```hcl
// main.tf
