...
各々の詳しい解説は[公式](https://aws.amazon.com/jp/blogs/news/under-the-hood-firelens-for-amazon-ecs-tasks/)に委ねて、ここでは全体像とFireLensとFluent Bitの役割について簡単に概要を解説するに留めます。
...
ECSエージェントがFireLensコンテナを起動するとFireLensは設定ファイルを生成します。
(生成されるファイルは後述します）
...
前提として`ecs-cli`を使っているので今回は`docker-compose.yml`と`ecs-params.yml`を修正していきます。タスク定義をjsonで記述している場合は[公式ドキュメント](https://docs.aws.amazon.com/ja_jp/AmazonECS/latest/developerguide/firelens-taskdef.html)を見てください。
...
シンタックスシュガーで実際はFluentedまたはFluent Bitのプライグインに変換されています。
